...
1. [ ] Verify analytics DB connectivity from service (ping query)
   - Link: [R4], [R5]
   - Depends: docker compose up (postgres healthy)


## Phase 1 — Schemas (must be first)
2. [ ] Add new Pydantic models in `analytics/schemas.py`
...
11. [ ] Usage endpoints in `analytics/routers/usage_analytics.py`
...
12. [ ] Conversation endpoints in `analytics/routers/conversation_analytics.py`
    - GET /analytics/conversation/no-answer-rate → NoAnswerStats
    - GET /analytics/conversation/citation-stats → CitationStats
