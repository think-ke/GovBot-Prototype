...
export function WebsiteManager() {
  ...
  const handleStartCrawl = async (event: React.FormEvent) => {
    ...
    setIsCrawling(true)
    try {
      // Simulate the crawl (replace with your API call later)
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Crawl websites and manage webpage content
        </p>
      </div>
        ...
        <form onSubmit={handleStartCrawl} className="space-y-4">
          ...
          <div>
            ...
            <p className="text-xs text-red-500 mt-1">
              * Required for organizing crawled webpages
            </p>
          </div>
          ...
          {crawlJobs.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-sm text-muted-foreground">
                Start your first website crawl above
              </p>
            </div>
          ) : (
            <div className="space-y-4">
              {crawlJobs.map((job) => (
                <div key={job.task_id} className="border rounded-lg p-4">
                  <div className="flex items-center justify-between">
                    <div className="flex-1">
                      ...
                      <div className="text-sm text-muted-foreground mt-1">
                        <span>Collection: {job.collection_id} • </span>
                        <span>URLs crawled: {job.urls_crawled}</span>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
